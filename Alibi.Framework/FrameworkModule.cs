...
namespace Alibi.Framework
{
    public class FrameworkModule : Autofac.Module
    {
        ...
        protected override void Load(ContainerBuilder builder)
        {
            ...
            builder.Register(c => new HttpContextAccessor()).As<IHttpContextAccessor>().SingleInstance();

            builder.RegisterGeneric(typeof(Repository<>)).As(typeof(IRepository<>)).InstancePerLifetimeScope();

            builder.Register(c => new TransactionInterceptor(c.Resolve<NHibernate.ISession>()));

            builder.Register(c => new AuthenticatedUserInterceptor(c.Resolve<IHttpContextAccessor>()));
